 ...
 *    * blockDim.x                any sensible thread block size
...
__global__ void cn_pnpoly(int* bitmap, float2* points, int n) {
    int i = blockIdx.x * blockDim.x * tile_size + threadIdx.x;
    if (i < n) {
        ...
        for (int ti=0; ti<tile_size; ti++) {
            c[ti] = 0;
            if (i+blockDim.x*ti < n) {
                lpoints[ti] = points[i+blockDim.x*ti];
            }
        }

        int k = VERTICES-1;

        #pragma unroll loop_unroll_factor_v
        ...
        for (int ti=0; ti<tile_size; ti++) {
            ...
            if (i+blockDim.x*ti < n) {
                #if use_method == 0
                bitmap[i+blockDim.x*ti] = c[ti];
                #elif use_method == 1
                bitmap[i+blockDim.x*ti] = c[ti] & 1;
                #endif
            }
        }
    }

...
RESULTS_LOC = os.path.join(BASE_DIR, "results", "small_cache_experiment_between_runs.json")
RESULTS_ITER_LOC = os.path.join(BASE_DIR, "results", "iter_results", "caching", "small_cache_results_between_runs.json")
...
def _calculate_speedup(cold, warm):
    ...
    return {
        "wallclock": ratio(cold.get("total_wallclock_mean", 1), warm.get("total_wallclock_mean", 1)),
        "compile": ratio(cold.get("total_compile_mean", 1), warm.get("total_compile_mean", 1))
    }
